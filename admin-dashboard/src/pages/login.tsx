...
export function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } else if (result.error === 'ERR_NETWORK' || result.error === 'ERR_TIMEOUT') {
      setStatus('error');
    ...
    <div className="min-h-screen bg-bg flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="fixed top-0 left-0 right-0 h-10 border-b border-borderBright bg-surface/80 backdrop-blur-sm flex items-center justify-between px-6 z-10">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-[0.55rem] uppercase tracking-widest text-alpha">BROADCAST LIVE</span>
        </div>
        <span className="font-mono text-xs text-textMuted tabular-nums">{hh}:{mm}:{ss}</span>
      ...
      <div className="w-full max-w-xs mb-8 mt-6">
        <div className="flex items-stretch border border-borderBright overflow-hidden">
          <div className="flex-1 bg-alpha/10 border-r border-borderBright py-2 px-3 text-center">
            <div className="font-display text-[0.7rem] font-bold text-alpha tracking-widest">ALPHA</div>
            <div className="font-mono text-[0.5rem] text-textDim mt-0.5">ATACANTE</div>
          </div>
          <div className="flex items-center justify-center px-4 bg-surface">
            <span className="font-display text-sm font-bold text-textDim tracking-widest">VS</span>
          </div>
          <div className="flex-1 bg-beta/10 border-l border-borderBright py-2 px-3 text-center">
            <div className="font-display text-[0.7rem] font-bold text-beta tracking-widest">BETA</div>
            <div className="font-mono text-[0.5rem] text-textDim mt-0.5">DEFENSOR</div>
          </div>
        </div>
      </div>
      ...
      <div className="w-full max-w-xs relative">
        ...
        <div className="border border-borderBright bg-surface p-8">
          ...
          <h1 className="font-display text-3xl font-bold uppercase tracking-[0.08em] text-text text-center mb-1">
      ...
      <div className="fixed bottom-0 left-0 right-0 h-7 border-t border-borderBright bg-surface2/80 overflow-hidden flex items-center">
        <div className="px-3 border-r border-borderBright shrink-0">
          <span className="font-mono text-[0.5rem] text-textDim uppercase tracking-widest">ARENA</span>
        </div>
        <div className="overflow-hidden flex-1 relative">
          <div className="ticker-wrap">
            <div className="ticker-content font-mono text-[0.5rem] text-textDim uppercase tracking-widest whitespace-nowrap">
              {[
                'DEBATE MODE ACTIVO', 'CHESS MODE DISPONIBLE', 'MULTI-PROVIDER SUPPORT',
                'ANTHROPIC · OPENROUTER · GROQ', 'SEASON 2025 EN CURSO',
                'DEBATE MODE ACTIVO', 'CHESS MODE DISPONIBLE', 'MULTI-PROVIDER SUPPORT',
                'ANTHROPIC · OPENROUTER · GROQ', 'SEASON 2025 EN CURSO',
              ].map((item, i) => (
                <span key={i} className="inline-block px-6">◆ {item}</span>
              ))}
            </div>
          </div>
        </div>
